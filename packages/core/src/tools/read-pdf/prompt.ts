...
export function readPdfPrompt(supportsVisual: boolean): string {
  const source = '读取会话附件时使用 sourceType=attachment、sourceValue=用户附加或 WebFetch 返回的附件 ID；读取项目内或已授权的本地 PDF 时使用 sourceType=path、sourceValue=路径。'
