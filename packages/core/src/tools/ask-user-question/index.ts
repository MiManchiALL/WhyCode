...
export function createAskUserQuestionTool(
  ...
  return buildTool({
    ...
    prompt:
      '仅当用户回答会实质改变下一步行动，且无法从现有上下文或只读工具查明时使用。若存在安全、合理且容易撤销的默认选择，应直接继续并说明假设。已有休眠任务计划时，如果最新用户只提出了另一个复杂目标、但没有明确是否用它覆盖旧计划，可以询问用户保留旧计划还是原子替换；用户已明确授权替换时不要重复确认。若当前请求已经可以完整交付，不得调用本工具；应正常完成，并在最终文本中说明可选方向。不得用本工具询问用户是否满意、是否还需要帮助，或对已经明确的恢复意图再次确认。一次只问一个清晰问题，提供 2~4 个互斥选项及具体取舍；不要添加“其它”，界面会自动提供自由输入。调用成功会暂停当前回合，等待用户回答。',
    inputSchema,
    isReadOnly: false,
    kind: 'control',
