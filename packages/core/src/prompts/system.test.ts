...
describe('通用 Agent 提示约束', () => {
  it('普通执行提示词保持主动协作、精确工具路由和最小改动约束', () => {
    ...
    assert.match(prompt, /对每个需要调用工具的用户请求，首次调用前必须用一小段话说明立即动作/)
    assert.doesNotMatch(prompt, /首次调用前始终用一句话/)
    assert.match(prompt, /首次调用之后，只在一批工作产生对用户有意义的实质进展时更新/)
    assert.match(prompt, /形成阶段结论、确认关键事实、完成成果、发现需要改变路线或遇到明确阻塞/)
    assert.match(prompt, /阶段更新用一小段话/)
    assert.match(prompt, /先说清已确认或已完成的内容及其影响，再按需说明接下来方向/)
    assert.match(prompt, /长任务不要让用户长时间没有反馈/)
    assert.match(prompt, /尚未形成完整结论时，也应基于实际完成范围、当前发现或明确等待来更新/)
    assert.match(prompt, /而不只预告下一步/)
    assert.match(prompt, /没有新的实质内容时直接继续调用工具/)
    assert.match(prompt, /不为每个工具或下一步计划单独发文字/)
    assert.match(prompt, /不重复未变化状态/)
    assert.match(prompt, /并在同一步继续调用工具/)
    assert.doesNotMatch(prompt, /单个工具动作/)
    assert.doesNotMatch(prompt, /1～2 句话/)
    assert.doesNotMatch(prompt, /有新进展的合理间隔/)
