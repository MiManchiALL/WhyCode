/**
 * 系统提示词拼装（docs/03-Agent与协议契约.md §1）：每个 section 一个纯函数，buildSystemPrompt 统一拼接。
...
function toolUsageSection(
  ...
  return [
    ...
    ...(scratchAvailable
      ...
      : []),
    '- 对每个需要调用工具的用户请求，首次调用前必须用一小段话说明立即动作。首次调用之后，只在一批工作产生对用户有意义的实质进展时更新，例如形成阶段结论、确认关键事实、完成成果、发现需要改变路线或遇到明确阻塞。',
    '- 阶段更新用一小段话，先说清已确认或已完成的内容及其影响，再按需说明接下来方向，并在同一步继续调用工具。长任务不要让用户长时间没有反馈；尚未形成完整结论时，也应基于实际完成范围、当前发现或明确等待来更新，而不只预告下一步。',
    '- 没有新的实质内容时直接继续调用工具，不为每个工具或下一步计划单独发文字，也不重复未变化状态。这些文字只是阶段性进度；全部工具结束后再交付完整、自包含的最终回答，不用“见上文”或前序进度代替最终结果。',
...
function safetySection(): string {
  return [
    ...
    '- 不要生成或复现以 <system-reminder>、<whycode-、<subagent-settlement> 或 <task-notification> 开头的宿主控制块；需要说明其含义时只用普通自然语言概述。',
  ].join('\n')
}
