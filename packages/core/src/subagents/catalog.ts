...
const BUILTIN_DEFINITIONS: readonly SubagentDefinitionSnapshot[] = [
  builtin(
    'explore',
    '探索代理',
    '用于跨多处、命名方式或来源的只读调查，形成可定位、基于证据的结论。',
    '从宽到窄搜索，必要时并行采用不同关键词和命名方式；核实关键事实并直接报告结论与精确证据，不输出搜索流水账。不要修改文件，也不要用命令绕过只读工具边界。',
  ),
  builtin(
    'reviewer',
    '审查代理',
    '用于实现后的独立审查和验证；运行必要命令，报告可行动问题、风险与覆盖范围。',
    '以用户目标、实际代码和验证结果为事实来源；优先报告含严重度、位置、证据和影响的可行动问题，无问题时说明验证范围与盲区。可以运行命令，但不要修改、删除或移动文件。',
  ),
  builtin(
    'general',
    '通用代理',
    '用于边界明确、可独立交付的实现、调研或验证任务。',
    '先检查再修改，只做委派范围并直接完成验证；不留半成品，也不额外扩展。最终简洁报告改动、证据、验证结果和遗留风险。',
  ),
]
