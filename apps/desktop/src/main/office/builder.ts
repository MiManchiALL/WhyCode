  ...
  async build(
    ...
    try {
      ...
      for (const [index, asset] of request.assets.entries()) {
        ...
        const size = await stageRegularFile(asset.path, assetPath, OFFICE_ARTIFACT_MAX_ASSET_BYTES)
      ...
      await publishVerifiedFile(outputPath, request.outputPath, result.inspection.sha256)
      return { outputPath: request.outputPath, inspection: result.inspection }
    } finally {
